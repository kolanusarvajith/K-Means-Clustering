...
def update_centers(data, labels, K):
    labels_matrix = np.zeros((len(data), K))
    labels_matrix[np.arange(len(data)), labels] = 1
    new_centers = np.dot(labels_matrix.T, data)/np.sum(labels_matrix, axis=0).reshape(K,1)
    return new_centers
